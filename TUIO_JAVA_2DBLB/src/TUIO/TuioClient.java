...
public class TuioClient implements OSCListener {
	...
	public void acceptMessage(Date date, OSCMessage message) {
		...
		/*
		//FIXME: print whole received OSC messages
		System.out.print("Received OSC message("+address+"): ");
		...
		*/
		...
		if (address.equals("/tuio/2Dobj")) {
			...
			if (command.equals("set")) {
				...
				System.out.println("tcur : ("+xpos+","+ypos+")");
			...
			// alive [s_id] ...include the session_id of every active blob 	
			} else if (command.equals("alive")) {
				...
				// remove the currently deleted cursors
					...
					while(frameEnum.hasMoreElements()) {
						...
						switch (tcur.getTuioState()) {
							case TuioCursor.TUIO_REMOVED:
								...
								// obtain the value for the max cursor id
								if (removeCursor.getCursorID()==maxCursorID) {
									...
									// freeCursorList contains a stack of TuioCursor objects available for usage
							...
							default:
								...
								// update the cursor already listed in the cursorList, if the position values changed, but it isn't moving.
								if ( (tcur.getX()!=updateCursor.getX() && tcur.getXSpeed()==0) || (tcur.getY()!=updateCursor.getY() && tcur.getYSpeed()==0) )
									...
									//FIXME: is this above really correct? 
		...
		//FIXME ******* start of modification ******
			...
			if (command.equals("set")) {
				...
				float xpos = ((Float)args[2]).floatValue();
				float ypos = ((Float)args[3]).floatValue();				
				float contactAngle = ((Float)args[4]).floatValue();
				float contactWidth = ((Float)args[5]).floatValue();
				float contactHeight = ((Float)args[6]).floatValue();
				float contactArea = ((Float)args[7]).floatValue();								
				...
				float x1=((Float)args[13]).floatValue();
				float y1=((Float)args[14]).floatValue();
				float x2=((Float)args[15]).floatValue();
				float y2=((Float)args[16]).floatValue();
				float x3=((Float)args[17]).floatValue();
				float y3=((Float)args[18]).floatValue();
				float x4=((Float)args[19]).floatValue();
				float y4=((Float)args[20]).floatValue();
				float x5=((Float)args[21]).floatValue();
				float y5=((Float)args[22]).floatValue();

				ArrayList<TuioPoint> puntos =new ArrayList<TuioPoint>();
				
				
				
				//BGUAMAN
				//Debugging existence of nulll coordinate
				if(x1 == 0){System.out.println("zero at X1");}
				else if(x2 == 0 ){System.out.println("zero at X2");}
				else if(x3 == 0 ){System.out.println("zero at X3");}
				else if (x4 == 0){System.out.println("zero at X4");}
				else if (x5 == 0){System.out.println("zero at X5");}
				else{
				...
				puntos.add(new TuioPoint(x1 , y1));
				puntos.add(new TuioPoint(x2 , y2));
				puntos.add(new TuioPoint(x3 , y3));
				puntos.add(new TuioPoint(x4 , y4));
				puntos.add(new TuioPoint(x5 , y5));
				
				
				if (blobList.get(s_id) == null) {
									
			
					TuioBlob addBlob = new TuioBlob(s_id, -1, xpos, ypos, contactAngle, contactWidth, contactHeight, contactArea, puntos);
					...
					if (tblb==null) return;
					if ((tblb.xpos!=xpos) || (tblb.ypos!=ypos) 
							|| (tblb.contactAngle!=contactAngle) || (tblb.contactWidth!=contactWidth) || (tblb.contactHeight!=contactHeight) || (tblb.contactArea!=contactArea)
							|| (tblb.x_speed!=xspeed) || (tblb.y_speed!=yspeed) || (tblb.rotation_speed !=aspeed) || (tblb.motion_accel!=maccel) ||  (tblb.rotation_accel!=raccel)) {

						TuioBlob updateBlob = new TuioBlob(s_id,tblb.getBlobID(),xpos,ypos,contactAngle,contactWidth,contactHeight,contactArea, puntos);
						updateBlob.update(xpos,ypos,xspeed,yspeed,maccel,puntos);
									
						//updateBlob.update(xpos,ypos,xspeed,yspeed,maccel,x1 ,  y1 ,  x2 ,  y2,
							//	 x3 ,  y3,  x4 ,  y4 ,  x5 ,  y5);
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------						
						case TuioBlob.TUIO_REMOVED:
								...
								if(removeBlob.getPuntos().get(0) == null  || removeBlob.getPuntos().get(1) == null || removeBlob.getPuntos().get(2) == null || removeBlob.getPuntos().get(3) == null ||removeBlob.getPuntos().get(4) == null ){
									System.out.println("null coordinate sent ");
									return; 
								...
								if (removeBlob.getBlobID()==maxBlobID) {
									...
									if (blobList.size()>0) {
										...
										while (blist.hasMoreElements()) {
											...
											if (c_id>maxCursorID) maxBlobID=c_id;
										}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
							case TuioBlob.TUIO_ADDED:
								...
								TuioBlob addBlob = new TuioBlob(currentTime,tblb.getSessionID(),c_id,tblb.getX(),tblb.getY(),tblb.getContactAngle(),tblb.getContactWidth(),tblb.getContactHeight(),tblb.getContactArea()
										,tblb.getPuntos());
...
//---------------------------------------------------------------------------------------------------------------------------------------------------
							default:
								...
								if ( (tblb.getX()!=updateBlob.getX() && tblb.getXSpeed()==0) || (tblb.getY()!=updateBlob.getY() && tblb.getYSpeed()==0) )
									updateBlob.update(currentTime,tblb.getX(),tblb.getY());
								else 
									updateBlob.update(currentTime,tblb.getX(),tblb.getY(),tblb.getXSpeed(),tblb.getYSpeed(),tblb.getMotionAccel());
								*/
								// ..this instead
								updateBlob.update(currentTime,tblb.getX(),tblb.getY(),tblb.getContactAngle(),tblb.getContactWidth(),tblb.getContactHeight(),tblb.getContactArea(),
										tblb.getPuntos());
